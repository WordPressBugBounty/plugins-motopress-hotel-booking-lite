...
/**
 * Route: /rates/{id}/custom_prices (GET)
 */
class GetRateCustomPricesController extends AbstractRestCommandController {
	...
	protected static function process_and_get_data_by_response_schema( \WP_REST_Request $request ) {
		...
		$prices = MPHB()->getCustomPricesRepository()->getPricesForPeriod( $rateId, $startDate, $endDate );

		/**
		 * PriceLabs Integration uses this hook to replace custom prices with
		 * its own data.
		 *
		 * @param array $prices <code>[ Date string ("Y-m-d") => Price (float) ]</code>
		 * @param int $rateId
		 * @param string $startDate
		 * @param string $endDate
		 */
		$prices = apply_filters( 'mphb_get_rate_custom_prices', $prices, $rateId, $startDate, $endDate );
